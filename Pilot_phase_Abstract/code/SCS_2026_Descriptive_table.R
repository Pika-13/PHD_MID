...
discrete_variables <- c(#"paper_type", 
                        ...
                        "type_of_study_1_ep_sup_non_inf",
                        "type_of_study_2_ep_sup_non_inf",
                        "ss_calculation", 
                        ...
                        "cci_ep_protocol",
                        "type_of_study_1_ep_sup_non_inf_protocol",
                        "type_of_study_2_ep_sup_non_inf_protocol",
...
RCT_matched_publications_protocols$type_of_study_1_ep_sup_non_inf <- factor(RCT_matched_publications_protocols$type_of_study_1_ep_sup_non_inf,
                                                                       levels = c("Superiority trial",
                                                                                  "Non-inferiority trial","Equivalence trial", "Not mentioned"))
RCT_matched_publications_protocols$type_of_study_2_ep_sup_non_inf <- factor(RCT_matched_publications_protocols$type_of_study_2_ep_sup_non_inf,
                                                                            levels = c("Superiority trial",
                                                                                       "Non-inferiority trial","Equivalence trial", "Not mentioned"))
...
RCT_matched_publications_protocols$conclusion_of_authors <- factor(RCT_matched_publications_protocols$conclusion_of_authors, 
                                             levels = c("Favorable to intervention",
                                                        " Unfavorable to intervention", "Not mentioned"))
...
RCT_matched_publications_protocols$study_type_mentioned_protocol <- factor(RCT_matched_publications_protocols$study_type_mentioned_protocol, 
                                                                           levels = c("Yes", "No") ,labels = c("Mentioned", "Not mentioned"))
RCT_matched_publications_protocols$type_of_study_1_ep_sup_non_inf_protocol <- factor(RCT_matched_publications_protocols$type_of_study_1_ep_sup_non_inf_protocol,
                                                                            levels = c("Superiority trial",
                                                                                       "Non-inferiority trial","Equivalence trial", "Not mentioned"))
RCT_matched_publications_protocols$type_of_study_2_ep_sup_non_inf_protocol <- factor(RCT_matched_publications_protocols$type_of_study_2_ep_sup_non_inf_protocol,
                                                                            levels = c("Superiority trial",
                                                                                       "Non-inferiority trial","Equivalence trial", "Not mentioned"))
...
var_list_total <- list(
 # protocol_published~"Pre-trial registration type",
  type_of_study_1_ep_sup_non_inf ~"Trial aim for primary endpoint explicited in published trial",
  type_of_study_2_ep_sup_non_inf~"Trial aim for secondary endpoint explicited in published trial",
  type_of_study_1_ep_sup_non_inf_protocol~"Trial aim for primary endpoint explicited in pre-trial registration",
  type_of_study_2_ep_sup_non_inf_protocol~"Trial aim for secondary endpoint explicited in pre-trial registration",
  ss_calculation_protocol~"Sample size calculation in pre-trial registration"
 # non_inf_margin_protocol~"Non-inferiority margin determined in pre-trial registration"
 )


#-----------------------------------

Table_pretrial_registration <- RCT_matched_publications_protocols %>%
  ...
  tbl_summary(
    ...
    missing = "no" ,
    digits = list(
      all_categorical() ~ c(0,0,0),
      all_continuous() ~ c(0,0,0)# 0 decimals for counts, 1 for percentages
    ))%>%
  as_flex_table()      %>% 
  add_footer_lines(values = "Discrete variables are expressed in percentage (number of observations/ total)
                     ")%>%
  add_footer_lines(values = "Abbreviations: RCT, Randomized Controlled Trial."
  ) %>% 
  set_caption("Table 1. Information provided in pre-trial registrations of published RCTs")
...
save_as_docx(Table_pretrial_registration, path = "tables/Table_pretrial_registration.docx")

#-----------------------------------
#-----------------------------------
RCT_publications <- read_csv("database/csv_files_R_coding/RCT_publications_2025_11_02.csv")
#ensure factor variables are factors
discrete_variables <- c(#"paper_type", 
  "protocol",
  #"protocol_published_original_column",
  ...
  #"definition_of_non_inf_margin",
  "protocol_published")
RCT_publications[, discrete_variables] <- lapply(
  RCT_publications[, discrete_variables], factor)

#ensure all continuous variables are numeric
continuous_variables <- c("sample_size", "between_group_difference_of_cci")
RCT_publications[, continuous_variables] <- lapply(
  RCT_publications[, continuous_variables], as.numeric)

RCT_publications$continent <- factor(RCT_publications$continent,
                                                       levels = names(sort(table(RCT_publications$continent)
                                                                           , decreasing = TRUE)))
RCT_publications$discipline = factor(RCT_publications$discipline, 
                                                       levels = names(sort(table(RCT_publications$discipline), 
                                                                           descending=TRUE)))

RCT_publications$protocol <- factor(RCT_publications$protocol, 
                                                      levels = c("Protocol", "Publication"))
RCT_publications$protocol_published <- factor(RCT_publications$protocol_published, 
                                                                levels = c("Protocol published", 
                                                                           "Protocol unpublished",
                                                                           "No protocol,registry"
                                                                           #, "Neither protocol nor registry"
                                                                ))
RCT_publications$study_type_pilot <- factor(RCT_publications$study_type_pilot, 
                                                              levels = c("Pilot/ Safety study", "Not pilot study"))
RCT_publications$pooled_analysis <- factor(RCT_publications$pooled_analysis, 
                                                             levels = c("Yes", "No", "Not mentioned"),labels = c("Yes", "No", "Not mentioned"))
RCT_publications$post_hoc_secondary_analysis <- factor(RCT_publications$post_hoc_secondary_analysis, 
                                                                         levels = c("Yes", "No", "Not mentioned"),labels = c("Yes", "No", "Not mentioned"))
RCT_publications$cci_ep <- factor(RCT_publications$cci_ep, 
                                                    levels = c("CCI primary endpoint", 
                                                               "CCI non-primary endpoint"
                                                               # "CCI not an endpoint"
                                                    ))
RCT_publications$cci_1st_ep = factor(RCT_publications$cci_1st_ep,
                                                       levels = c("Yes", "No", "Not mentioned"),labels = c("Yes", "No", "Not mentioned"))
RCT_publications$cci_2nd_ep = factor(RCT_publications$cci_2nd_ep,
                                                       levels = c("Yes", "No", "Not mentioned"),labels = c("Yes", "No", "Not mentioned"))
RCT_publications$cci_expl_ep = factor(RCT_publications$cci_expl_ep,
                                                        levels = c("Yes", "No", "Not mentioned"),labels = c("Yes", "No", "Not mentioned"))
RCT_publications$cd_cci_reported_in_results <- factor(RCT_publications$cd_cci_reported_in_results, 
                                                                        levels = c("Yes", "No", "Not mentioned"),labels = c("Reported", "Not reported", "Not mentioned"))
RCT_publications$cci_modification <- factor(RCT_publications$cci_modification, 
                                                              levels = c("Yes", "No", "Not mentioned"),labels = c("Modified", "Not modified", "Not mentioned"))
RCT_publications$primary_outcome_significant <- factor(RCT_publications$primary_outcome_significant, 
                                                                         levels = c("Yes", "No"),labels = c("Significant", "Non-significant"))
RCT_publications$statistically_significant_difference_for_cci <- factor(RCT_publications$statistically_significant_difference_for_cci, 
                                                                                          levels = c("Yes", "No"),labels = c("Significant", "Non-significant"))
RCT_publications$between_group_difference_of_cci_12 <- factor(RCT_publications$between_group_difference_of_cci_12, 
                                                                                levels = c("Yes", "No"),labels = c("≥ 12 pts", "< 12 pts"))
RCT_publications$mean_or_median_used <- factor(RCT_publications$mean_or_median_used, 
                                                                 levels = c("Mean", "Median"))
RCT_publications$study_type_mentioned <- factor(RCT_publications$study_type_mentioned, 
                                                                  levels = c("Yes", "No"),labels = c("Mentioned", "Not mentioned"))
RCT_publications$type_of_study_1_ep_sup_non_inf <- factor(RCT_publications$type_of_study_1_ep_sup_non_inf,
                                                                            levels = c("Superiority trial",
                                                                                       "Non-inferiority trial","Equivalence trial", "Not mentioned"))
RCT_publications$type_of_study_2_ep_sup_non_inf <- factor(RCT_publications$type_of_study_2_ep_sup_non_inf,
                                                                            levels = c("Superiority trial",
                                                                                       "Non-inferiority trial","Equivalence trial", "Not mentioned"))

RCT_publications$ss_calculation <- factor(RCT_publications$ss_calculation, 
                                                            levels = c("Yes", "No"),labels = c("Yes", "No"))
RCT_publications$cci_used_for_ss_calculation <- factor(RCT_publications$cci_used_for_ss_calculation, 
                                                                         levels = c("Yes", "No"),labels = c("Used", "Not used"))
RCT_publications$non_inf_margin <- factor(RCT_publications$non_inf_margin, 
                                                            levels = c("Yes", "No"),labels = c("Defined", "Not defined"))
RCT_publications$significance_considered <- factor(RCT_publications$significance_considered, 
                                                                     levels =  c("Statistical significance", "Both statistical and clinical significance"))
RCT_publications$conclusion_of_authors <- factor(RCT_publications$conclusion_of_authors, 
                                                                   levels = c("Favorable to intervention",
                                                                              " Unfavorable to intervention", "Not mentioned"))
RCT_publications$cci_ep_protocol <- factor(RCT_publications$cci_ep_protocol, 
                                                             levels = c("CCI primary endpoint", 
                                                                        "CCI non-primary endpoint",
                                                                        "CCI not an endpoint"
                                                             ))
RCT_publications$study_type_mentioned_protocol <- factor(RCT_publications$study_type_mentioned_protocol, 
                                                                           levels = c("Yes", "No") ,labels = c("Mentioned", "Not mentioned"))
RCT_publications$type_of_study_1_ep_sup_non_inf_protocol <- factor(RCT_publications$type_of_study_1_ep_sup_non_inf_protocol,
                                                                                     levels = c("Superiority trial",
                                                                                                "Non-inferiority trial","Equivalence trial", "Not mentioned"))
RCT_publications$type_of_study_2_ep_sup_non_inf_protocol <- factor(RCT_publications$type_of_study_2_ep_sup_non_inf_protocol,
                                                                                     levels = c("Superiority trial",
                                                                                                "Non-inferiority trial","Equivalence trial", "Not mentioned"))

RCT_publications$ss_calculation_protocol <- factor(RCT_publications$ss_calculation_protocol, 
                                                                     levels = c("Yes", "No") ,labels = c("Calculated", "Not calculated"))
RCT_publications$cci_used_for_ss_calculation_protocol <- factor(RCT_publications$cci_used_for_ss_calculation_protocol, 
                                                                                  levels = c("Yes", "No"),labels = c("Used", "Not used"))
RCT_publications$non_inf_margin_protocol <- factor(RCT_publications$non_inf_margin_protocol, 
                                                                     levels = c("Yes", "No"),labels = c("Defined", "Not defined"))

#variable labels
#List with all renamed variables
var_list_significance <- list(
  primary_outcome_significant ~ "Primary outcome significant",
  cci_ep~"CCI as an endpoint",
  significance_considered ~"Significance considered",
  conclusion_of_authors ~"Conclusion favours intervention",
  type_of_study_1_ep_sup_non_inf~"Study aim for primary endpoint",
  type_of_study_2_ep_sup_non_inf~"Study aim for secondary endpoint",
  ss_calculation~"Sample size calculation",
   non_inf_margin~"Defined non-inferiority margin"
)

Table_pretrial_significance <- RCT_publications %>%
  select(primary_outcome_significant,
         cci_ep,
         significance_considered,
         conclusion_of_authors,
         type_of_study_1_ep_sup_non_inf,
  ...
  tbl_summary(
  #  by = protocol_published,
    label = var_list_significance,
    statistic = list(
       c("primary_outcome_significant" ,
       "cci_ep",
        "significance_considered",
         "conclusion_of_authors",
        "type_of_study_1_ep_sup_non_inf",
  ...
  as_flex_table()      %>% 
  add_footer_lines(values = "Discrete variables are expressed in percentage (number of observations/ total non-missing)
                     ")%>%
  add_footer_lines(values = "Abbreviations: RCT, Randomized Controlled Trial."
  ) %>% 
  set_caption("Table 1. Significance of published RCTs")

#add_overall()
#add_p() %>%
#Save Table_descriptive
save_as_docx(Table_pretrial_significance, path = "tables/Table_pretrial_significance.docx")
...
mismatch_cci_table <- 
  ...
  tbl_summary(
    by = cci_ep ,
    label = cci_ep_protocol ~"CCI(R) as an endpoint in pre-trial registrations",
    statistic = list(
...
RCT_cci_1_ep <- read_csv("database/csv_files_R_coding/RCT_cci_1_ep_2025_11_02.csv")
#List with all renamed variables
